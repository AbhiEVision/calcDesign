...
function RemoveUserImage(id){
    let x = FetchImages();
    const Index = x.findIndex(x=> x.id = id);
    if(Index != -1){
        x = x.filter(function(data){
            return data.id != id;
        })
    }
    StoreImagesInLocalStorage(x);
}